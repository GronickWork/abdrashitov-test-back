...
export default function Replica({chirp}) {
  const { auth } = usePage().props;
  const [ editing, setEditing ] = useState(false);
  const {data, setData, post, clearErrors, processing, errors, reset} = useForm({message: ''});

  function submit(e) {
      e.preventDefault();
      post(route('blogs.update', chirp.id), { onSuccess: () => setEditing(false) });
  }
  return (
                ...
                {chirp.user_id === auth.user.id && (
                  <Dropdown>
                    ...
                    <Dropdown.Content>
                        ...
                        <Dropdown.Link as="button" method="delete" href={route('blogs.destroy', chirp.id)}>
                            Delete
                        </Dropdown.Link>
                    </Dropdown.Content>
                  </Dropdown>
